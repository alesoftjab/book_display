...
//~ console.log(o.rows);		
	...
	let formatoFecha = {year: 'numeric', month: '2-digit', day: '2-digit'};  //, hour: '2-digit', minute: '2-digit'

	let inputArray = (o);
	let outputArray = [];

	let cols = inputArray.cols;
	let rows = inputArray.rows;
	for (let i = 0; i < rows.length; i++) {
		newRow = [];
		for (let j = 0; j < cols.length; j++) {
			let nombre_col = cols[j].label; 
			let valor = rows[i]['c'][j]['f'];
			if(cols[j].type == 'number')
			  valor = valor.toLocaleString("es",  {style: "decimal", useGrouping: true, minimumFractionDigits: "2", maximumFractionDigits: "2"});
			if(cols[j].type == 'date')
			  valor = new Date(valor).toLocaleDateString("es-AR", formatoFecha);
			let celda={}; 
			celda[nombre_col] = valor; 
			newRow.push(celda);
		}
		newRow1 = {c: newRow};
		outputArray.push(newRow1);
	}
